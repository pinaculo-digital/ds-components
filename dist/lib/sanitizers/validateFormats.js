...
class ValidateInputs {
     ...
     */
    string(string, tamanhoMinimo) {
        if (string)
            return string.length >= tamanhoMinimo;
        return false;
    }
    /**
    ...
    telefone(telefone) {
        ...
        if (tel.length === 10 || tel.length === 11) {
            // Verifica se é um número de telefone fixo (10 dígitos) que começa com 2
            if (tel.length === 10 && tel.charAt(2) === '2') {
                return true;
            }
            // Verifica se é um número de celular (11 dígitos) que começa com 9
            else if (tel.length === 11 && tel.charAt(2) === '9') {
                return true;
            }
        }
        // Caso não atenda às condições acima, retorna false
        return false;
    }
    /**
