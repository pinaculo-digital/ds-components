 ...
 */
export function action(key, action, event) {
    if (event.key === key) {
        action();
    }
}
export const colorOpacity = (c, opacity) => {
    let r = 0, g = 0, b = 0;
...
export const effectBy = (deps, fn) => {
    let prevDepValues = null;
    $effect(() => {
        fn(prevDepValues);
...
export function formatarRedeSocial(url) {
...
export const validarSeSenhasSaoIguais = (password, repeatPassword) => {
...
export function onMountWithSkeleton(fn) {
    onMount(() => {
        let done = false;
        (async () => {
            startSkeleton();
