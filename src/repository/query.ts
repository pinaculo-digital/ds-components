...
class Query {
  ...
  async setParamAndRefresh(
    ...
    await this.replaceHistory(forceRefresh ?? false, newUrl);
  }
  ...
  private async replaceHistory(forceRefresh: boolean, newUrl: string) {
    if (!forceRefresh) {
      ...
      await window.history.replaceState({ replaceState: true }, "", newUrl);
