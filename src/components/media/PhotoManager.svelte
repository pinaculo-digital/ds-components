    ...
    <div class="flex {tema === 'normal' ? 'flex-col' : ''}">
      ...
      {#if data64}
        <div class="mt-2 flex flex-wrap items-center gap-3">
          <Button color="red-300" onClick={() => (data64 = '')} label=">Remover" />
          <Button color="red-300" onClick={triggerFileUpload} label="Trocar imagem" />
        </div>
      {:else}
        <div class="mt-2 flex items-center">
          <Button color="red-300" onClick={triggerFileUpload} label="Adicionar imagem" />
        </div>
      {/if}
    </div>
  </div>
