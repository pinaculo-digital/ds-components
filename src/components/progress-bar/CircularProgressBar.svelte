<script lang="ts">
  interface Props {
    ...
    size?: number;
    displayerType?: 'percentage' | string;
  }

  const radius = 45;
  const circumference = 2 * Math.PI * radius;

  let { percentage = $bindable(5), size = 60, hasDisplayer = true, displayerType = 'percentage' }: Props = $props();
...
<div class="relative flex aspect-square items-center justify-center" style={`width: ${size}px; height: ${size}px;`}>
  <svg class="h-full w-full -rotate-90 transform" viewBox="0 0 100 100">
    <circle cx="50" cy="50" r={radius} stroke="var(--neutral-200)" stroke-width="10" fill="transparent" />
