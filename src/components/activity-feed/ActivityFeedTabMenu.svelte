...
<script lang="ts">
  const defaultProps: Props = {
    ...
    dividers: [
      [
        { label: 'Atividade', count: 1 },
        { label: 'Progresso', count: 2 },
      ],
      [
        { label: 'Leitura', count: 3 },
        { label: 'Lugares', count: 4 },
      ],
      [
        { label: 'Viagem', count: 5 },
        { label: 'Fotos', count: 6 },
      ],
    ],
  };

  let { activeTab = $bindable(defaultProps.activeTab), dividers = defaultProps.dividers }: Props = $props();
</script>

<div class="border-soft-200 bg-white-0 flex items-center gap-2 border-b">
